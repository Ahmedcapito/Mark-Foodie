    ...
    <div className="mb-20 mt-30">
      <div className="flex justify-center items-center gap-5">
        <input
          ...
          className="cursor-pointer bg-gray-100 w-170 ml-10 h-15 text-center rounded-sm font-bold text-lg border-none outline-none"
        />
        <div className="p-2 bg-amber-600 rounded-4xl cursor-pointer">
          <svg
      ...
      <div className="grid grid-cols-1 mt-20 sm:grid-cols-2 md:grid-cols-3 gap-6 m-auto px-20">
        {results.length > 0 ? (
          results.map((item, index) => (
            <motion.div
              key={`${item.id}-${index}`}
              className="bg-white p-4 rounded-lg w-90 shadow-md"
